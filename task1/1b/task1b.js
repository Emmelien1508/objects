...
function createMenuAndOrderOptions() {
    ...
    for (const [item, price] of Object.entries(menuItems)) {
        menu.innerHTML += `<div class="menu-item"><p>${item}</p> <p>$${price.toFixed(2)}</p></div>`;
...
function showOrderItems() {
    ...
    for (const [item, quantity] of Object.entries(cart)) {
        summary.innerHTML += `<div class="order-summary-item"><p>${item}</p><p>${quantity}</p><p>$${getItemTotal(item).toFixed(2)}</p></div>`;
    }
